...
def rankHand(hand):
    counts = {}
    for i in range(2, 10):
        counts[str(i)] = hand.count(str(i))
    for c in ["A", "K", "Q", "T"]:
        counts[c] = hand.count(c)
    sortedHand = "".join(sorted(hand.replace("J", max(counts, key=counts.get))))
    # five of a kind
    if re.search(r"([AKQT2-9])\1\1\1\1", sortedHand):
        return 7
    # four of a kind
    if re.search(r"([AKQT2-9])\1\1\1", sortedHand):
        return 6
    # full house
    if re.search(r"([AKQT2-9])\1\1([AKQJT2-9])\2", sortedHand) or re.search(
        r"([AKQJT2-9])\1([AKQJT2-9])\2\2", sortedHand
    ):
        return 5
    # three of a kind
    if re.search(r"([AKQT2-9])\1\1", sortedHand):
        return 4
    # two pair
    elif re.search(r"([AKQT2-9])\1.*([AKQJT2-9])\2", sortedHand):
        return 3
    # one pair
    elif re.search(r"([AKQT2-9]).*\1", sortedHand):
        return 2
    # high card
    else:
        return 1
...
def compareCards(x, y):
    faceCards = {"A": 14, "K": 13, "Q": 12, "J": 1, "T": 10}
    if x in faceCards:
        numX = faceCards[x]
    else:
        numX = int(x)
